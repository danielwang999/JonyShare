  ...
  export default {
    ...
    methods: {
      ...
      save() {
        ...
        Loading.show();
        _this.$ajax.post(process.env.VUE_APP_SERVER + '/system/admin/member/save', _this.member).then((response)=>{
          Loading.hide();
          let resp = response.data;
          if (resp.success) {
      ...
      del(id) {
        ...
        Confirm.show("删除会员后不可恢复，确认删除？", function () {
          Loading.show();
          _this.$ajax.delete(process.env.VUE_APP_SERVER + '/system/admin/member/delete/' + id).then((response)=>{
            Loading.hide();
            let resp = response.data;
            if (resp.success) {
              _this.list(1);
